...
func (pq *PollQuery) QueryCreatedBy() *UserQuery {
...
func (pq *PollQuery) WithCreatedBy(opts ...func(*UserQuery)) *PollQuery {
	query := (&UserClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PollQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Poll, error) {
	...
	if query := pq.withCreatedBy; query != nil {
		if err := pq.loadCreatedBy(ctx, query, nodes, nil,
			func(n *Poll, e *User) { n.Edges.CreatedBy = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PollQuery) loadPollOptions(ctx context.Context, query *PollOptionQuery, nodes []*Poll, init func(*Poll), assign func(*Poll, *PollOption)) error {
	...
	for _, n := range neighbors {
		fk := n.poll_poll_options
		if fk == nil {
			return fmt.Errorf(`foreign-key "poll_poll_options" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "poll_poll_options" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (pq *PollQuery) loadCreatedBy(ctx context.Context, query *UserQuery, nodes []*Poll, init func(*Poll), assign func(*Poll, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Poll)
	for i := range nodes {
		if nodes[i].user_created_polls == nil {
			continue
		}
		fk := *nodes[i].user_created_polls
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_created_polls" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
