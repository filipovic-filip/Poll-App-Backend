...
func (e PollEdges) CreatedByOrErr() (*User, error) {
	if e.loadedTypes[1] {
		if e.CreatedBy == nil {
...
func (*Poll) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case poll.FieldName, poll.FieldDescription:
			values[i] = new(sql.NullString)
		case poll.ForeignKeys[0]: // user_created_polls
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case poll.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field user_created_polls", value)
			} else if value.Valid {
				po.user_created_polls = new(int)
				*po.user_created_polls = int(value.Int64)
			}
		default:
...
// QueryCreatedBy queries the "created_by" edge of the Poll entity.
func (po *Poll) QueryCreatedBy() *UserQuery {
	return NewPollClient(po.config).QueryCreatedBy(po)
}
