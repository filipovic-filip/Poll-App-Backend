...
func (uc *UserCreate) AddCreatedPollIDs(ids ...int) *UserCreate {
	uc.mutation.AddCreatedPollIDs(ids...)
	return uc
}

// AddCreatedPolls adds the "created_polls" edges to the Poll entity.
func (uc *UserCreate) AddCreatedPolls(p ...*Poll) *UserCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return uc.AddCreatedPollIDs(ids...)
}
...
func (uc *UserCreate) AddVotes(p ...*PollOption) *UserCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.CreatedPollsIDs(); len(nodes) > 0 {
