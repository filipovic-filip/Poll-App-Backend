...
// QueryUsersVoted chains the current query on the "users_voted" edge.
func (poq *PollOptionQuery) QueryUsersVoted() *UserQuery {
...
func (poq *PollOptionQuery) Clone() *PollOptionQuery {
	...
	return &PollOptionQuery{
		...
		withUsersVoted: poq.withUsersVoted.Clone(),
...
// WithUsersVoted tells the query-builder to eager-load the nodes that are connected to
// the "users_voted" edge. The optional arguments are used to configure the query builder of the edge.
func (poq *PollOptionQuery) WithUsersVoted(opts ...func(*UserQuery)) *PollOptionQuery {
	query := (&UserClient{config: poq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	poq.withUsersVoted = query
	return poq
}
...
func (poq *PollOptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PollOption, error) {
	...
	if query := poq.withUsersVoted; query != nil {
		if err := poq.loadUsersVoted(ctx, query, nodes,
			func(n *PollOption) { n.Edges.UsersVoted = []*User{} },
			func(n *PollOption, e *User) { n.Edges.UsersVoted = append(n.Edges.UsersVoted, e) }); err != nil {
			return nil, err
		}
	}
...
func (poq *PollOptionQuery) loadUsersVoted(ctx context.Context, query *UserQuery, nodes []*PollOption, init func(*PollOption), assign func(*PollOption, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(polloption.UsersVotedTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(polloption.UsersVotedPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(polloption.UsersVotedPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(polloption.UsersVotedPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "users_voted" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (poq *PollOptionQuery) loadPoll(ctx context.Context, query *PollQuery, nodes []*PollOption, init func(*PollOption), assign func(*PollOption, *Poll)) error {
	...
	for i := range nodes {
		if nodes[i].poll_poll_options == nil {
			continue
		}
		fk := *nodes[i].poll_poll_options
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "poll_poll_options" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
